...
# This test only installs aws-efs-utils, mounts the existing EFS into a job-private
# subdirectory, runs basic file checks and cleans that subdirectory up afterwards.
...
sub run {
    ...
    # Smoke test the bundled helper binary, if present
    ...
    my $mount_efs = '/usr/sbin/mount.efs';
    $instance->ssh_assert_script_run("test -f $mount_efs");
    ...
    $instance->ssh_assert_script_run('systemctl status mnt-efs.mount || echo "NOP:$?"');
