...
sub run {
    ...
    if ($provider_cloud_name eq 'EC2') {
        ...
        my $instance_id = aws_vm_get_id($region, $job_id);
        ...
        assert_script_run("aws ec2 delete-security-group --group-id " . aws_security_group_get_id($region, $job_id) . " --region $region");
        assert_script_run("aws ec2 delete-subnet --subnet-id " . aws_subnet_get_id($region, $job_id) . " --region $region");
        my $igw_id = aws_internet_gateway_get_id($region, $job_id);
