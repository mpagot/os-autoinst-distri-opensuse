...
subtest '[aws_create_internet_gateway]' => sub {
    my $awscli = Test::MockModule->new('sles4sap::aws_cli', no_auto => 1);
    my @calls;
    $awscli->redefine(script_output => sub { push @calls, $_[0]; return 'igw-123'; });
    $awscli->redefine(assert_script_run => sub { push @calls, $_[0]; return; });

    my $res = aws_create_internet_gateway(
        region => 'us-west-1',
        job_id => '67890'
    );
...
subtest '[aws_get_internet_gateway_id]' => sub {
    my $awscli = Test::MockModule->new('sles4sap::aws_cli', no_auto => 1);
    my @calls;
    $awscli->redefine(script_output => sub { push @calls, $_[0]; return 'igw-123'; });

    my $res = aws_get_internet_gateway_id(
        region => 'us-west-1',
        job_id => '67890'
    );
...
subtest '[aws_attach_internet_gateway]' => sub {
    my $awscli = Test::MockModule->new('sles4sap::aws_cli', no_auto => 1);
    my @calls;
    $awscli->redefine(assert_script_run => sub { push @calls, $_[0]; return; });
