...
sub crash_destroy_aws(%args) {
    ...
    assert_script_run("aws ec2 delete-subnet --subnet-id " . aws_subnet_get_id(region => $args{region}, job_id => $job_id) . " --region $args{region}");
    my $igw_id = aws_internet_gateway_get_id(region => $args{region}, job_id => $job_id);
    script_run("aws ec2 detach-internet-gateway --vpc-id $vpc_id --internet-gateway-id $igw_id --region $args{region}");
    script_run("aws ec2 delete-internet-gateway --internet-gateway-id $igw_id --region $args{region}");
    my $rtb_ids = script_output(
        "aws ec2 describe-route-tables --filters Name=vpc-id,Values=$vpc_id" .
          " --query 'RouteTables[?Associations[0].Main!=\`true\`].RouteTableId'" .
          " --output text --region $args{region}");
    assert_script_run("aws ec2 delete-route-table --route-table-id $_ --region $args{region}") for split(/\s+/, $rtb_ids);

    # Delete everything else (AWS handles dependencies automatically if we wait)
    script_run("aws ec2 delete-vpc --vpc-id $vpc_id --region $args{region}");
}
