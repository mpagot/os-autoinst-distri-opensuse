...
sub aws_vpc_create(%args) {
    ...
    assert_script_run("aws ec2 create-tags --resources $vpc_id --tags Key=OpenQAJobVpc,Value=$args{job_id} --region $args{region}");
    return $vpc_id;
}
...
=head2 aws_create_internet_gateway
    ...
    my $igw_id = aws_create_internet_gateway(
        region => 'us-west-1',
        job_id => '67890'
    );
...
sub aws_create_internet_gateway(%args) {
    foreach (qw(region job_id)) {
        croak("Argument < $_ > missing") unless $args{$_};
    }
...
=head2 aws_get_internet_gateway_id
    ...
    my $igw_id = aws_get_internet_gateway_id(
        region => 'us-west-1',
        job_id => '67890'
    );
...
sub aws_get_internet_gateway_id(%args) {
    foreach (qw(region job_id)) {
        croak("Argument < $_ > missing") unless $args{$_};
    }
    return script_output(join(' ',
...
sub aws_attach_internet_gateway(%args) {
...
=head2 aws_route_tablei_associate
